...
// function upFunction(){
	...
	// allowed = true;
	// rotating = false;
	// console.log('rotazione', rotating);
...
// function getMIDIMessage(midiMessage) {
	...
	// if (midiMessage.data[0]==224 && midiMessage.data[2]!=64 && midiMessage.data[1]!=0){
		
		// newRotating=true;
		// // console.log("pitchBend", midiMessage.data[2], Date.now()-currentTime)
		// if (midiMessage.data[2]>65) {
			// afterTouch=(midiMessage.data[2] - 65);
			// bendUp=-1;
			
		// } else {
			// afterTouch=(65 - midiMessage.data[2]);
			// bendUp=1;
	...
	// } else if (midiMessage.data[1]==midiNote && midiMessage.data[0]==128){
			// eventLines.push('<br>'+'<span style="background: #ff0;">'+midiMessage.data.toString()+'</span>');
			// upFunction();
	// } else if (midiMessage.data[2]==64 && midiMessage.data[1]==0 && midiMessage.data[0]==224 && newRotating==false){
			// downFunction(127);
			// upFunction();
	// } else if (midiMessage.data[2]==64 && midiMessage.data[1]==0 && midiMessage.data[0]==224 && newRotating==true){
			// upFunction();
			// newRotating=false;
